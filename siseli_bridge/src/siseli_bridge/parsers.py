...
def _quick_decode_ps4z_state(blocks: dict) -> dict:
    ...
    # PS4Z_CLEAN_FINAL_MAPPING: final direct mapping from decoded PS4Z/Sgx0 blocks
    try:
        _ps4z_raw = blocks.get("PS4Z") if isinstance(blocks, dict) else None
        _sgx0_raw = blocks.get("Sgx0") if isinstance(blocks, dict) else None

        if _ps4z_raw:
            _p = _u16le_regs_from_modbus_block(_ps4z_raw)
            if len(_p) > 4:
                _pv1_v = round(float(_p[3]) / 10.0, 1)
                _pv1_w = float(_p[4])
                if 0 <= _pv1_v <= 600 and 0 <= _pv1_w <= 30000:
                    out["pv_v"] = _pv1_v
                    out["pv_w"] = round(_pv1_w, 1)
                    out["pv_current_a"] = round(_pv1_w / _pv1_v, 2) if _pv1_v > 0 else 0

        if _sgx0_raw:
            _s = _u16le_regs_from_modbus_block(_sgx0_raw)
            if len(_s) > 28:
                _pv2_v = round(float(_s[27]) / 10.0, 1)
                _pv2_w = float(_s[28])
                if 0 <= _pv2_v <= 600 and 0 <= _pv2_w <= 30000:
                    out["pv2_v"] = _pv2_v
